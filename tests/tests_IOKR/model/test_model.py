...
from stpredictions.models.IOKR.model import IOKR as iokr
from stpredictions.datasets.load_data import load_bibtex, load_corel5k
from stpredictions.models.IOKR.utils import project_root
...
def fitted_predicted_IOKR(
    ...
    results = {"Y_train": Y_train, "Y_test": Y_test, "Y_pred_test": Y_pred_test}
    return results
...
class TestFit:
    ...
    def test_verbose(self, capfd):
        ...
        """Test if fit function actually prints something"""
        scores = fitted_predicted_IOKR(
            DATASETS["bibtex"]["X"], DATASETS["bibtex"]["Y"], L=1e-5
        )
        assert scores  # XXX
        out, err = capfd.readouterr()
        assert err == "", f"{err}: need to be fixed"
        assert out != "", "Fitting Time should have been printed "
    ...
    def test_time(self, nameTree, Tree, nameSet, dataXY, L=1e-5):
        ...
        """
        X_train, X_test, Y_train, Y_test = train_test_split(
            dataXY["X"], dataXY["Y"], test_size=0.33, random_state=42
        )
        clf = Tree()
        clf.verbose = 1
        t0 = time.time()
    ...
    def test_fit_fits(self, nameTree, Tree, nameSet, dataXY):
        ...
        assert np.array_equal(
            clf1, clf2
        ), f"Failed with {nameTree}/{nameSet}: 'clf' should be fitted after fitting"
...
class TestPredict:
    ...
    def test_model_return_values(self, nameTree, Tree, nameSet, dataXY):
        ...
        None
        """
        returned_IOKR = fitted_predicted_IOKR(
            dataXY["X"],
            dataXY["Y"],
            L=1e-5,
        )
        ...
        assert isinstance(
            returned_IOKR["Y_test"], np.ndarray
        ), f"Failed with {nameTree}/{nameSet}:'Y_test' should be a 'np.ndarray, but is {type(returned_IOKR['Y_test'])}"
        assert isinstance(
            returned_IOKR["Y_pred_test"], np.ndarray
        ), f("Failed with {nameTree}/{nameSet}:'Y_pred_test' should be a 'np.ndarray, but is "
             "{type(returned_IOKR['Y_pred_test'])}")
    ...
    def test_recall(self, nameTree, Tree, nameSet, dataXY):
        ...
        None
        """
        fp_IOKR = fitted_predicted_IOKR(
            dataXY["X"],
            dataXY["Y"],
            L=1e-5,
        )
        recall_test = recall_score(
            fp_IOKR["Y_test"], fp_IOKR["Y_pred_test"], average="micro"
        )
        threshold = 0
        assert recall_test > threshold, (
            f"Failed with {nameTree}/{nameSet}: recall_test = {recall_test},"
    ...
    def test_precision(self, nameTree, Tree, nameSet, dataXY):
        ...
        threshold = 0
        assert precision_test > threshold, (
            f"Failed with {nameTree}/{nameSet}: precision_test = {precision_test},"
            f"but threshold set to {threshold}"
        )
